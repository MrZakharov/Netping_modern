            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    var cont = String.Format(@"<html><h2>{0}</h2>
                        ...
                        {6}</table>
                        </html>", cart.Name,  cart.Address, cart.IndexCode, cart.Phone, cart.Requisites, sum, items);
                    ...
                    cont = String.Format(@"<html><h2>Ваш заказ был получен, ждите ответа менеджера нашей компании</h2>
                        ...
                        <tr><th></th><th>Наименование</th><th>Цена</th><th>Количество</th><th>Стоимость</th></tr>
                        {6}</table>
                        </html>", cart.Name, cart.Address, string.IsNullOrEmpty(cart.Shipping) ? "--" : cart.Shipping, cart.Phone, cart.Requisites, sum, items, cart.IndexCode);
